...
export default function WholesaleProductsPage() {
  ...
  const addMinimumToCart = async (product: Product) => {
    ...
    try {
      ...
      await addToCart(user.uid, product.id, product.name, product.wholesalePrice || product.price, product.thumbnail || product.images?.[0] || '', quantity);
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
        ...
        <div className="grid gap-4 py-6 sm:grid-cols-2 xl:grid-cols-3">{filtered.map((product) => {
          ...
          return <article key={product.id} className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl">
            <Link href={`/products/${product.id}`} className="relative block h-48 overflow-hidden bg-slate-100"><Image src={product.thumbnail || product.images?.[0] || '/images/Groceries1.png'} alt={product.name} fill className="object-cover transition duration-500 group-hover:scale-105"/><span className="absolute left-3 top-3 rounded-full bg-slate-950/80 px-2.5 py-1 text-xs font-bold text-white">MOQ {moq} {product.unit || 'units'}</span></Link>
