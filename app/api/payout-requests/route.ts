...
export async function GET(request: NextRequest) {
  try {
    ...
    const db = getAdminDb();
    const snapshot = canOperateFinance(user)
    ...
    return NextResponse.json({ requests: snapshot.docs.map((document) => ({ id: document.id, ...document.data() })) });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const db = getAdminDb();
    const [profileSnapshot, openDispute] = await Promise.all([
      db.collection('payoutProfiles').doc(user!.uid).get(),
      db.collection('disputes').where('sellerIds', 'array-contains', user!.uid).where('active', '==', true).limit(1).get(),
    ]);
  ...
  try {
    ...
    await db.runTransaction(async (transaction) => {
      ...
      if (action === 'approve' && ['pending_approval', 'exception_review'].includes(payout.status)) {
        ...
        transaction.update(ref, { approvalIds: approvals, status: approved ? 'approved' : payout.status, updatedAt: now });
